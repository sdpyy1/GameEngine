...
namespace Hazel {
	...
	void createBuffer(VkPhysicalDevice physicalDevice,VkDevice device, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
	{
		...
		bufferInfo.size = size;   // 申请缓冲区的大小
		bufferInfo.usage = usage; 	 // 这个缓冲区是要作为顶点缓冲区使用
		bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;// 这个缓冲区在图像队列族中独占使用
		...
		VkMemoryAllocateInfo allocInfo{};
		allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
		allocInfo.allocationSize = memRequirements.size; // 分配内存的大小
		// 寻找合适的内存索引  VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT表示这种内存可以被 CPU（主机）访问 ，VK_MEMORY_PROPERTY_HOST_COHERENT_BIT表示这种内存具有 “一致性”，即 CPU 对内存的修改会自动同步到 GPU
		allocInfo.memoryTypeIndex = findMemoryType(physicalDevice,memRequirements.memoryTypeBits, properties);
	...
	void transitionImageLayout(VkQueue graphicsQueue, VkCommandPool commandPool, VkDevice device, VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
	{
		...
		barrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED; // 这里也可以修改队列族
	...
	void endSingleTimeCommands(VkQueue graphicsQueue, VkDevice device, VkCommandPool commandPool, VkCommandBuffer commandBuffer)
	{
		...
		// 该commandBuffer是一次性命令使用的，使用完就可以释放
