...
namespace Hazel {
	...
	void OpenGLRendererAPI::DrawIndexed(const Ref_old<VertexArray>& vertexArray, uint32_t indexCount)
	{
		vertexArray->Bind();
	...
	void OpenGLRendererAPI::DrawLines(const Ref_old<VertexArray>& vertexArray, uint32_t vertexCount)
	{
		vertexArray->Bind();
