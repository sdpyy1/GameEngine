...
namespace GameEngine {
    ...
    public:
        ...
        struct Key
        {
            Key(const VulkanUtil::VulkanRenderPassAttachments& info)
        ...
        ~VkRenderPassCache() { Clear(); }

        CachedRenderPass Allocate(const VulkanUtil::VulkanRenderPassAttachments& info);

        inline uint32_t CachedSize() { return cachedPasses.size(); }
        void Clear();

    private:
        std::unordered_map<Key, CachedRenderPass, Key::Hash> cachedPasses;
    };
    ...
    public:
        ...
        ~VkFramebufferCache() { Clear(); }

        CachedFramebuffer Allocate(const VkFramebufferCreateInfo& info);
        // void Release(const CachedFramebuffer& pooledFramebuffer);

        inline uint32_t CachedSize() { return cachedFramebuffers.size(); }
        void Clear();

    private:
        std::unordered_map<Key, CachedFramebuffer, Key::Hash> cachedFramebuffers;
