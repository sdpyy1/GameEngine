...
namespace Hazel {
	...
	void VulkanImage2D::RT_Invalidate()
	{
		...
		imageCreateInfo.tiling = m_Specification.Usage == ImageUsage::HostRead ? VK_IMAGE_TILING_LINEAR : VK_IMAGE_TILING_OPTIMAL; // �ڴ沼��
		...
		else if (m_Specification.Usage == ImageUsage::HostRead) // VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL
		{
			...
			VulkanContext::GetCurrentDevice()->FlushCommandBuffer(commandBuffer); // ��������ύ���ȴ����
		}
		else // Ĭ����ֻ������
		{
			VkCommandBuffer commandBuffer = VulkanContext::GetCurrentDevice()->GetCommandBuffer(true);
			...
			Utils::InsertImageMemoryBarrier(commandBuffer, m_Info.Image,
				...
				VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT | VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, // ��ɫ���׶�
				subresourceRange);
