...
namespace Hazel {
	...
	public:
		...
		uint32_t GetParentBoneIndex(const uint32_t boneIndex) const { ASSERT(boneIndex < m_ParentBoneIndices.size(), "bone index out of range in Skeleton::GetParentIndex()!"); return m_ParentBoneIndices[boneIndex]; }
		...
		const std::string& GetBoneName(const uint32_t boneIndex) const { ASSERT(boneIndex < m_BoneNames.size(), "bone index out of range in Skeleton::GetBoneName()!"); return m_BoneNames[boneIndex]; }
